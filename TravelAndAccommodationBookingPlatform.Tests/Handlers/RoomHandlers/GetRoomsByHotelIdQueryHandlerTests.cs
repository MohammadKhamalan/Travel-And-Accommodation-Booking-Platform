...
{
    [Fact]
    public async Task Handle_ShouldReturnRoomsForHotel()
    {
        // Arrange
        var repoMock = new Mock<IRoomRepository>();
        var mapperMock = new Mock<IMapper>();
        var handler = new GetRoomsByHotelIdQueryHandler(repoMock.Object, mapperMock.Object);

        var rooms = new List<Room> { new Room { AdultsCapacity = 2, ChildrenCapacity = 1, Rating = 4.5f } };
        var roomDtos = new List<RoomDto> { new RoomDto { AdultsCapacity = 2, ChildrenCapacity = 1, Rating = 4.5f } };
        var paged = new PaginatedList<Room>(rooms, 1, 10, 1);

        repoMock.Setup(r => r.GetRoomsByHotelIdAsync(It.IsAny<Guid>(), null, 1, 10)).ReturnsAsync(paged);
        ...
        result.Items[0].Rating.Should().Be(4.5f);
    }
}
